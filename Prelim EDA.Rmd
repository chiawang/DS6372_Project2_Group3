...
```{r}
f <- as.formula(outcome ~ discharge_disposition_id + time_in_hospital + num_lab_procedures + num_medications + number_emergency + number_inpatient + diag_1 + diag_2 + number_diagnoses + metformin + diabetesMed + ageGrp)
...
```{r}
f <- as.formula(outcome ~ race + admission_type_id + discharge_disposition_id + admission_source_id + time_in_hospital + num_lab_procedures + num_medications + number_emergency + number_inpatient + diag_1 + diag_2 + diag_3 + number_diagnoses + A1Cresult + metformin + glimepiride + acarbose + diabetesMed + ageGrp + I(number_inpatient^.5))
...
Notice above we got a better training score with the complext model, we ended up getting a lower score when applying to the test set.
...
### HeatMap
```{r}
ggplot(data = data,aes(x = data$race, y = data$ageGrp)) +labs(title="(Race & Age) Vs Readmission", y="Patient Age Group", x="Race")+geom_tile(aes(fill = data$outcome))

ggplot(data = data, aes(x = data$time_in_hospital, y = data$ageGrp)) + labs(title="(Time in hospital & Age) Vs Readmission", y="Patient Age Group", x="Time in hospital") + geom_tile(aes(fill = data$outcome))

ggplot(data = data, aes(x = data$ageGrp, y = data$diag_1)) + labs(title="(Age & Diag_1) Vs Readmission", y="Patient Age Group", x="Daig_1") + geom_tile(aes(fill = data$outcome))

ggplot(data = data, aes(x = data$ageGrp, y = data$diag_2)) + labs(title="(Age & Diag_2) Vs Readmission", y="Patient Age Group", x="Daig_2") + geom_tile(aes(fill = data$outcome))

ggplot(data = data, aes(x = data$ageGrp, y = data$diag_3)) + labs(title="(Age & Diag_3) Vs Readmission", y="Patient Age Group", x="Daig_3") + geom_tile(aes(fill = data$outcome))
