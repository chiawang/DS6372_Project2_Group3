...
Run model against test set, using simple fit, no interaction
```{r}
#Run model from training set on valid set I
f <- as.formula(outcome ~ .)
test.x <- model.matrix(f, test)
...
### Complex Logistic Example, 1 interaction
```{r}
#glmnet requires a matrix
f <- as.formula(outcome ~ . + diabetesMed*diag_1)
train.x <- model.matrix(f, train)
#train.x <- data.matrix(train.x)
cvfit <- cv.glmnet(train.x,train.y, family = "binomial", type.measure = "class", nlambda = 1000)
plot(cvfit)
...
Run model against test set, with interaction
```{r}
#Run model from training set on valid set I
f <- as.formula(outcome ~ . + change*diag_1)
test.x <- model.matrix(f, test)
#test.x <- data.matrix(test.x)
fit.pred1 <- predict(cvfit, newx = test.x, type = "response")
